...
void sobel24BitsMultiCpu(unsigned char *originalImageSegment, unsigned char *edgeImageSegment,
...
void DetectEdgesMultiCpu(BITMAPINFO *bmpInfo, void *bmpData)
{
	...
	sImageMultiCpu originalImage, edgeImage;
	...
	for (int i = 0; i < numChunks; i++)
	{
		for (int j = 0; j < originalImage.rows; j++)
		{
			...
			for (int k = 0; k < chunkSize; k++)
			{
				...
				sobel24BitsMultiCpu(originalImage.rdata, edgeImage.rdata, originalImage.rows, originalImage.cols, i, chunkSize, k, j);
				//Run algorithm for green color.
				sobel24BitsMultiCpu(originalImage.gdata, edgeImage.gdata, originalImage.rows, originalImage.cols, i, chunkSize, k, j);
				//Run algorithm for blue color.
				sobel24BitsMultiCpu(originalImage.bdata, edgeImage.bdata, originalImage.rows, originalImage.cols, i, chunkSize, k, j);
			}
		}
	}
